...
it('can generate APIM token using cache first', function () {
    $apimToken = app(Token::class)->getApimToken();

    expect($apimToken)->toHaveProperty('access_token');
});

it('can clear cache and generate APIM token using cache first', function () {
    $apimToken = app(Token::class)->getNewApimToken();

    expect($apimToken)->toHaveProperty('access_token');
});

it('can generate SSO token using cache first', function () {
    $ssoToken = app(Token::class)->getSsoToken();

    expect($ssoToken)->toHaveProperty('access_token');
});

it('can clear cache and generate SSO token using cache first', function () {
...
it('can generate APIM token that differs from cache', function () {
...
it('can generate SSO token that differs from cache', function () {
